...
function renderLegend(color, data) {
  ...
  legend.enter()
        ...
        .merge(legend)
          .attr("width", "20")
          .attr("height", "20")
          .attr("y", function(d, i) { return (svg_height-400) - 25*i; })
          .attr("x", 150)
  ...
  text.data(legend_items)
    .enter().append("text").merge(text)
      .attr("y", function(d, i) { return (svg_height-384) - 25*i; })
      .attr("x", 180)
...
function calcColorScale(data) {
  ...
  let colorSet = ["#12e741","#d8f015","#ec9d09","#df0404","#9200c5","#6b0000"];
  let colorSet2 =["#f7fbff","#deebf7","#c6dbef","#9ecae1","#6baed6","#4292c6","#2171b5","#08519c","#08306b"];

  let scale = d3.scaleQuantile()
    .domain([0, 55, 155, 255, 355, 425, 800])
    .range(colorSet);
